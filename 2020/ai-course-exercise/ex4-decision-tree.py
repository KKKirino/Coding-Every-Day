  ...
  def build_children(self) -> Tuple[DecisionTreeNode, DecisionTreeNode]:
    ...
    for col in range(self.values.shape[1]):
      ...
      for split_index in range(len(val_range) - 1):
        # 3. split subset according to the split point
    ...
    # 6. bulid left child and right child with max split point
...
class DecisionTree:
  ...
  def prune(self, X: np.ndarray = None, y: np.ndarray = None) -> None: # TODO
    """Prune the tree with Reduce-Error Pruning (REP). If no (X, y) given,use training data."""
